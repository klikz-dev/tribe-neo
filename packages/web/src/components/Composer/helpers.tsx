...
export const getDOMContent = (domNode: any): any => {
  ...
  if (domNode.attribs?.href && domNode.attribs?.rel) {
    return (
      <a
        className="text-actionPrimary-600"
  ...
  if (domNode.attribs?.['data-type'] === 'mention') {
    ...
    return <ComposerMention id={mentionId} title={getTextContent(domNode)} />
  }
...
export const getLinkContent = ({ data, type, name: TagName, ...rest }) => {
  ...
  return (
    <TagName className={tagClassName[TagName]}>{getDOMContent(rest)}</TagName>
  )
}
...
export const checkNavigationAllowance =
  (quillRef: RefObject<ComposerRefImperativeHandle>) => event => {
